...
export class DepJumpProvider implements vscode.DefinitionProvider {
  ...
  provideDefinition(document: vscode.TextDocument, position: vscode.Position) {
    ...
    const targetDirection = this.#getDir(document.uri.fsPath, word);

    if (verifyExistAndNotDirectory(targetDirection))
      return new vscode.Location(vscode.Uri.file(targetDirection), POSITION);
  }
}
